...
$scratch = "/home/hildilab/dev/ext";
$blast_db = "/home/hildilab/data/ext/blast";
$rosdir = "$scratch/rosetta/rosetta_bin_linux_2018.09.60072_bundle";
$fragsDir = "$rosdir/rosetta/rosetta_fragments_latest";

my $pathToDemo="$rosdir/demos/protocol_capture/flex_pep_dock_abinitio/";
    
# psi-blast
#my $BLAST_BIN_DIR = "$shareware_dir/new_blast/blast-2.2.12/bin";
my $PSIBLAST = "blastpgp";     # PSI-BLAST (duh.)
my $NR = "$blast_db/nr";                # nr blast database filename
...
my $VALL_BLAST_DB = "$rosdir/tools/fragment_tools/vall.jul19.2011.blast";
my $BLOSUM = "$rosdir/main/database/sequence/substitution_matrix/";

# psipred
my $FILTNR = "$blast_db/uniref90filt";        # filtnr blast database filename
my $MAKEMAT = "makemat";       # makemat utility (part of NCBI tools)
my $PSIPRED = "$scratch/psipred/bin/psipred";       # psipred
my $PSIPASS2 = "$scratch/psipred/bin/psipass2";     # psipass2 (part of psipred pkg)
my $PSIPRED_DATA = "$scratch/psipred/data";         # dir containing psipred data files.

# nnmake
my $VALL = "$rosdir/tools/fragment_tools/";          # dir containing vall database (cvs repository 'nnmake_database')
#my $VALL2 = "$scratch/$fragsDir/nnmake_database";             # alt dir containing vall database (cvs repository 'nnmake_database')
my $VALL2 = "$rosdir/main/database/sampling/";
my $VALL_NAME = "filtered.vall.dat.2006-05-05";                 # filename of vall (vall.dat.<id> and vall_cst_coord.dat.<id> must exist)
...
my $JUFO = "$scratch/bcl/bin/bcl-apps-static.exe";                              # jufo executable
...
# run blast
unless (&fileExist("$id$chain.chk" ))  {
    #print "$PSIBLAST -t T -i $file -F F -j2 -o $id$chain.blast -d $NR -v10000 -b10000 -K1000 -h0.0009 -e0.0009 -C $id$chain.chk -Q $id$chain.pssm\n";
  if (!&try_try_again("$PSIBLAST -t T -i $file -F F -j2 -o $id$chain.blast -d $NR -v10000 -b10000 -K1000 -h0.0009 -e0.0009 -C $id$chain.chk -Q $id$chain.pssm",
		      2, ["$id$chain.chk"], ["$id$chain.chk","$id$chain.blast","$id$chain.pssm","error.log"])) {
...
unless (&fileExist("$id$chain.checkpoint")) {
  ...
  @checkpoint_matrix = &parse_checkpoint_file("$id$chain.chk");
...
if ($no_homs) {
  unless (&fileExist("$id$chain.outn")) {
    if (!&try_try_again("$PSIBLAST -t T -i $file -j 1 -R $id$chain.chk -o $id$chain.outn -e 0.05 -d $VALL_BLAST_DB", 2, ["$id$chain.outn"], ["$id$chain.outn"])) {
